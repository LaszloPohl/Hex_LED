...
//***********************************************************************
    ...
    //***********************************************************************
    matrix<adattipus> YA, YB_NZB, XA, XB;   // fw be
...
//***********************************************************************
    ...
    if (p_faelem_adat->cella_index != 0) { // cellára kapcsolódik
        ...
        if (is_szimm) {
            YA.set_size_szimm_and_zero(p_faelem_adat->A);
            YRED.set_size_szimm(p_faelem_adat->A);
        }
        else {
            YA.set_size_and_zero(p_faelem_adat->A, p_faelem_adat->A);
            YRED.set_size(p_faelem_adat->A, p_faelem_adat->A);
        }
        ...
        for (uns j = 0; j < mas.size(); j++) {
            ...
            if (sor.is_masolando) { 
                if (sor.is_balbol) { // másolandónál csak az egyikbõl másol 
                    if (osz.is_masolando) {
                        if (osz.is_balbol)   YA.submatrix_copy(p_bal->YRED, sor.hova, osz.hova, sor.bal_from, osz.bal_from, sor.db, osz.db); // mindkettõ másolandó, YA-ba megy a blokk
                        else                 ; // mindkettõ másolandó, de nem ugyanaz a forrás, így ilyenkor nincs mûvelet
                    }
                    else {
                        if (osz.is_centroid) YA.submatrix_copy(p_bal->YRED, sor.hova, osz.hova, sor.bal_from, osz.bal_from, sor.db, osz.db); // sor másolandó, oszlop centroid: másolás YA-ba
                    ...
                    if (osz.is_masolando) {
                        ...
                        else                 YA.submatrix_copy(p_jobb->YRED, sor.hova, osz.hova, sor.jobb_from, osz.jobb_from, sor.db, osz.db); // mindkettõ másolandó, YA-ba megy a blokk
                    }
                    else {
                        if (osz.is_centroid) YA.submatrix_copy(p_jobb->YRED, sor.hova, osz.hova, sor.jobb_from, osz.jobb_from, sor.db, osz.db); // sor másolandó, oszlop centroid: másolás YA-ba
                ...
                if (sor.is_centroid) { 
                    if (osz.is_masolando) {
                        if (osz.is_balbol)   YA.submatrix_copy(p_bal->YRED, sor.hova, osz.hova, sor.bal_from, osz.bal_from, sor.db, osz.db); // sor centroid, oszlop másolandó: YA-ba másolja
                        else                 YA.submatrix_copy(p_jobb->YRED, sor.hova, osz.hova, sor.jobb_from, osz.jobb_from, sor.db, osz.db); // sor centroid, oszlop másolandó: YA-ba másolja
                    }
                    else { // oszlop nem másolandó
                        if (osz.is_centroid) YA.submatrix_add(p_bal->YRED, p_jobb->YRED, sor.hova, osz.hova, sor.bal_from, osz.bal_from, sor.jobb_from, osz.jobb_from, sor.db, osz.db); // mindkettõ centroid: YA-ba teszi a két blokk összegét
...
//***********************************************************************
    if (YB_NZB.get_col() == 1) {
        ...
        YRED.math_1_add_mul_symm(YA, XB, NZBXA);
    }
    else if (YB_NZB.get_col() == 2) {
        NZBXA.math_2_ninv_mul_symm(YB_NZB, XA);
        YRED.math_2_add_mul_symm(YA, XB, NZBXA);
    }
    else {
...
//***********************************************************************
    if (YB_NZB.get_col() == 1) {
        NZBXA.math_1_ninv_mul(YB_NZB, XA);
        YRED.math_1_add_mul(YA, XB, NZBXA);
    }
    else if (YB_NZB.get_col() == 2) {
        NZBXA.math_2_ninv_mul(YB_NZB, XA);
        YRED.math_2_add_mul(YA, XB, NZBXA);
    }
    else {
        YB_NZB.math_ninv_np();
        XAT.transp(XA);
        ...
        NZBXAT.transp(NZBXA);
        YRED.math_add_mul_t(YA, XB, NZBXAT);
    }
}
